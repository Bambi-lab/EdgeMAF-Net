...
class ClassifyMetrics(SimpleClass):
    """
    Class for computing classification metrics including accuracy, precision, recall, F1-score, and specificity.

    Attributes:
        accuracy (float): Overall accuracy.
        precision (float): Macro-averaged precision.
        recall (float): Macro-averaged recall (sensitivity).
        f1_score (float): Macro-averaged F1-score.
        specificity (float): Macro-averaged specificity.
        speed (Dict[str, float]): A dictionary containing the time taken for each step in the pipeline.

    Properties:
        fitness (float): The fitness of the model, calculated as mean of accuracy and f1_score.
    ...
    def process(self, targets, pred):
        ...
        self.accuracy = (pred_top1 == targets).float().mean().item()
        
        # Get number of classes
        num_classes = max(targets.max().item(), pred_top1.max().item()) + 1
        ...
        for cls in range(num_classes):
            ...
            tp = ((pred_top1 == cls) & (targets == cls)).sum().item()
            fp = ((pred_top1 == cls) & (targets != cls)).sum().item()
            fn = ((pred_top1 != cls) & (targets == cls)).sum().item()
            tn = ((pred_top1 != cls) & (targets != cls)).sum().item()
        ...
        self.precision = sum(precision_list) / len(precision_list) if precision_list else 0.0
        self.recall = sum(recall_list) / len(recall_list) if recall_list else 0.0
        self.specificity = sum(specificity_list) / len(specificity_list) if specificity_list else 0.0
    ...
    @property
    def fitness(self):
        """Returns mean of accuracy and F1-score as fitness score."""
        return (self.accuracy + self.f1_score) / 2

    @property
    def results_dict(self):
        """Returns a dictionary with model's performance metrics and fitness score."""
        return dict(zip(self.keys + ["fitness"], 
                       [self.accuracy, self.precision, self.recall, self.f1_score, self.specificity, self.fitness]))

    @property
    def keys(self):
        """Returns a list of keys for the results_dict property."""
        return ["metrics/accuracy", "metrics/precision", "metrics/recall", "metrics/f1_score", "metrics/specificity"]
