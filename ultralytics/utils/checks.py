    ...
    if ONLINE and is_pip_package():
        with contextlib.suppress(Exception):
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    f"New EdgeMAF-Net version {latest} available 😃 "
                    f"Check for updates at your project repository"
                )
                return True
    return False
...
def check_amp(model):
    ...
    prefix = colorstr("AMP: ")
    LOGGER.info(f"{prefix}running Automatic Mixed Precision (AMP) checks with EdgeMAF-Net...")
    warning_msg = "Setting 'amp=True'. If you experience zero-mAP or NaN losses you can disable AMP with amp=False."
    
    try:
        # 使用传入的模型进行简单的 AMP 测试，不加载额外的权重
        im = torch.randn(1, 3, 640, 640).to(device)  # 创建随机输入
        ...
        assert a.shape == b.shape, "AMP output shape mismatch"
        LOGGER.info(f"{prefix}checks passed ✅")
    except ConnectionError:
        LOGGER.warning(f"{prefix}checks skipped ⚠️, offline and unable to download model. {warning_msg}")
    except (AttributeError, ModuleNotFoundError):
        LOGGER.warning(
            f"{prefix}checks skipped ⚠️. "
            f"Unable to load model due to possible package modifications. {warning_msg}"
        )
